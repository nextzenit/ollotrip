...
$router->get('account/activation/(\d+)/(\d+)', function($user_id,$email_code) {
    ...
    if(empty($res->data)){
        echo "User already activated or wrong credentials";
        die;
    }
...
// ======================================================== User account delete GET
$router->get('(delete-account)', function($menu) {
    ...
    views($meta,"Accounts/Delete-account");
...
// ======================================================== REPORTS GET
$router->get('(reports)/(\d+)', function($menu,$year) {
    ...
    $booking_data = POST(api_url.'user_bookings', $params);

    $booking = [];

    foreach ($booking_data->data->flights as $value) {
        $booking = booking_report($booking, $value, 'flights', 1);
    }
    foreach ($booking_data->data->hotels as $value) {
        $booking = booking_report($booking, $value, 'hotel', 1);
    }
    foreach ($booking_data->data->tours as $value) {
        $booking = booking_report($booking, $value, 'tours', 1);
    }
    foreach ($booking_data->data->cars as $value) {
        $booking = booking_report($booking, $value, 'cars', 1);
    }

    $meta['data'] = $booking;
    $meta['reports_active']=$menu;
    $meta['year']=$year;
    views($meta, "Accounts/Reports");

});

//Report Function
function booking_report($booking, $value, $type, $number) {
    if($value->booking_status == "confirmed" && $value->payment_status == "paid") {
        $fee = !empty($value->agent_fee) ? $value->agent_fee : 0;

        $searchDate = $value->booking_date;

        $resultObjects = array_filter($booking, function ($object) use ($searchDate) {
            return $object->booking_date == $searchDate;
        });

        $resultObjects = array_values($resultObjects);

        if (!empty($resultObjects)) {
            $resultObjects[0]->{$type . '_sales'} += $value->price_markup;
            $resultObjects[0]->{$type . '_booked'} += 1;
            $resultObjects[0]->{$type . '_commission'} += $fee;
        } else {
            $booking[] = (object)[
                $type . '_booked' => $number,
                $type . '_sales' => $value->price_markup,
                $type . '_commission' => $fee,
