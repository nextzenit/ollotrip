...
$router->post('cars/search', function () {
    ...
    if(!empty($m_cars)){
        $response = array("status" => true, "message" => "MANUAL CARS", "response" => $m_cars,'total'=>count($m_cars));
    } else {
        $response = array("status" => false, "message" => "No Cars data", "response" => (object)[],'total'=>'');
    }
