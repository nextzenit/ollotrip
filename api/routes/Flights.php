    ...
    if (!empty($Multithreading)) {
        foreach ($Multithreading as $key => $value) {
            ...
            $param = array(
                ...
                'infants' => $infants,
                'class' => $class,
                'endpoint' => api_modules ."/flights/". strtolower($value['name']) ."/api/v1/search",
    ...
    if (!empty($data)) {
        ...
        if($getvalue[0]['prn_type'] == $_POST['booking_type']) {
            $param = array(
                ...
                'c5' => $getvalue[0]['c5'],
                'evn' => $evn,
                'endpoint' =>  api_modules . "/flights/".strtolower($data[0]['supplier'])."/api/v1/booking",
                //'endpoint' => "http://localhost/modules/flights/" . strtolower($data[0]['supplier']) . "/api/v1/booking",
